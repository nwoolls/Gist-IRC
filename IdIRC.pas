        ...
        case PosInStrArray(LCTCP, IdIRCCTCP) of
          0: { ACTION }
            begin
              ...
              //BEGIN NATE - why would we send an error for channel actions??
...
var
  LTmp, LNick, LChannel, LTopic: String;
begin
  if Assigned(FOnTopic) then
  begin
    //BEGIN NATE
    if ASender.CommandHandler.Command = '332' then
    begin
      LTmp := ASender.UnparsedParams;
      LNick := FetchIRCParam(LTmp);
      LChannel := FetchIRCParam(LTmp);
      LTopic := FetchIRCParam(LTmp);
      OnTopic(ASender.Context, LNick, '', LChannel, LTopic);
    end else
    begin
    //END NATE
      if ASender.Params.Count > 0 then begin
...
//BEGIN NATE
    FetchIRCParam(LTmp);
    OnServerWelcome(ASender.Context, FetchIRCParam(LTmp) + ' ' + LTmp);
//END NATE
  end;
end;
...
begin
  ...
  if ASender.Params.Count < 3 then
    FSvrList.Add(ASender.Params[0] + ' ' + ASender.Params[1]) {do not localize}
  else
  //END NATE
